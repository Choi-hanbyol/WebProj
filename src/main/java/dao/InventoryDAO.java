...
public class InventoryDAO {
	...
	public ArrayList<InventoryDTO> history(String dd) {
		...
		InventoryDTO idto;
		sql = "SELECT i_index, (SELECT s_name FROM subject_info WHERE s_index = i_index) AS i_name, i_cnt, i_type, i_date FROM inventory_management WHERE DATE_FORMAT(I_DATE, '%Y-%m-%d') = ?";
		try {
			ptmt = con.prepareStatement(sql);
			ptmt.setString(1, dd);
